...
### Baseline: “memory-off”
1. Let device relax (≫ phonon lifetime) to erase residuals.  
2. Inject **N** identical probe pulses with phase sweep; record outputs → **P₀(φ)**, visibility **V₀(φ)**.

### Write controlled memory
3. Apply **write** pulse train through the SBS/phonon section with two randomized conditions:  
...
### Readout
4. Immediately (within τ_phonon) inject identical low-energy **probe** pulses; sweep/hold phase φ.  
5. Record outputs for each condition: **P+(φ)** and **P−(φ)**.  
6. **Interleave** trials (W+, W−, blank) in randomized order to suppress drift.
...
- **Entropy shift ΔH:** compare output distribution entropies **H(P₀)** vs **H(P±)**; expect bounded, repeatable **ΔH > 0** in the predicted direction for W+.  
...
- **Null model:** \( P_+ = P_- = P_0 \) within bounds from shot + detector noise + calibrated drift.  
...
Standard thermo-optic/nonlinear shifts don’t **label** the past once you equalize power and timing.  
Here we **encode and retrieve condition-specific traces** (W+ vs W−) that persist across probes and vanish on erasure schedules matching independent acoustic/phonon lifetimes.  
That’s a **history-conditioned bias**—the targeted signature.
...
- Standard resources on integrated SBS/Brillouin photonics and on-chip MZIs/HOM interferometry.

---

## 11) Probability and Risk Assessment
To evaluate the feasibility of proceeding with physical verification, we modelled the expected signal strength (ΔV — change in interference visibility) and experimental sensitivity under standard laboratory conditions.

| Hypothesized effect size | Detection power (10⁵–10⁶ counts/condition) |
|---------------------------|---------------------------------------------|
| ≥ 0.5 % ΔV | 85 – 95 % |
| 0.2 – 0.5 % ΔV | 50 – 75 % |
| ≤ 0.1 % ΔV | 10 – 30 % |

Factoring realistic experimental noise and a neutral prior belief (~30 % likelihood that Verrell’s Law–type bias exists at ≥ 0.2 % amplitude), the overall probability of observing a statistically significant positive result in the first campaign is **≈ 25 – 35 %**.  
With optimized hardware (high-Q SBS/phonon section, long τ, sub-mK thermal stability, blind randomized sequencing), this rises to **≈ 50 – 65 %**.

### Interpretation
These values indicate a **favorable risk-to-reward ratio** for proceeding with the experiment.  
Even the conservative model yields a non-trivial success probability, while a positive outcome would represent a decisive advance in testing the memory-bias hypothesis.  
Accordingly, we judge the experimental risk **acceptable** and the prospective information gain **high**.

---

### Contact
For coordination, email the Verrell’s Law project team (Collapse Aware AI).  
